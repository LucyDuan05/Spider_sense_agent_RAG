...
def weibull_tailfitting(meanfiles_path, distancefiles_path,
    ...
    for filename in os.listdir(meanfiles_path):
        ...
        distance_file_npz = os.path.join(distancefiles_path, category + ".npz")
        distance_file_npy = os.path.join(distancefiles_path, category + ".npy")
        if os.path.isfile(distance_file_npz):
            distance_scores = np.load(distance_file_npz)[distance_type]
        else:
            distance_scores = np.load(distance_file_npy, allow_pickle=True)[()][distance_type]
