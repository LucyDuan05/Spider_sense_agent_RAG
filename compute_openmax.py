...
def recalibrate_scores(weibull_model, img_features,
    ...
    alpharank = min(alpharank, NCLASSES)
    ranked_list = img_features[:NCLASSES].argsort().ravel()[::-1]
    ...
    for cls_indx in range(NCLASSES):
        ...
        category_weibull = query_weibull(str(cls_indx), weibull_model, distance_type = distance_type)
...
def get_scores(data_type, weibull_model, feature_path, alpharank, distance_type='eucos'):
